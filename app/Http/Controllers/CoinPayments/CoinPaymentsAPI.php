...
*/
namespace App\Http\Controllers\CoinPayments;
...
class CoinPaymentsAPI
{
	 ...
	 */
	public function CreateTransactionSimple($username,$member_email,$amount, $currency1, $currency2, $address='', $ipn_url='') {

	//$address='mrYzP5u8eE8pRBpjMc6sKZUMrmAs5ddrPR';
		$req = array(
			'amount' => $amount,
			'currency1' => $currency1,
			'currency2' => $currency2,
	...
	public function CreateTransaction($req) {
		// See https://www.coinpayments.net/merchant-tools-api for parameters
		return $this->api_call('create_transaction', $req);
	}
	 ...
	 */
	public function CreateWithdrawal($amount, $currency, $address, $auto_confirm = FALSE) {
		$req = array(
			'amount' => $amount,
			'currency' => $currency,
			'address' => $address,
